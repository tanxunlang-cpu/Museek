## 2026-09-25 - A play URL whose host is unreachable never wins, and never hangs

Decision:
Play-URL liveness lives in `src/lib/hostHealth.ts`: `markHostDead` records a host after a probe fails at the connection level (DNS, refused, unreachable) and `isHostKnownDead` fails closed for ten minutes, while `markHostLive` / `isKnownCdnHost` mark hosts that may be *preferred*. `fetchContentLength` in `src/lib/audioUrlProbe.ts` classifies its errors with `isConnectionLevelError` — which deliberately excludes timeouts — returns a `DEAD` sentinel instead of `null` when both the HEAD and the ranged GET never reached an origin, and `looksLikeRealAudio` rejects a URL on a dead host and marks a host live once it serves real bytes. `raceMusicUrlWave` settles immediately on a trusted host (proven live, or one of the known platform CDNs) but only holds an unknown one for `GRACE_MS` (1.5s) while a better candidate is still in flight. `AudioPlayer.awaitHtmlPlayback` bounds a start with a 6s stall budget (20s for asset/blob URLs) and rejects with `AUDIO_STALLED` when the element never gets past `readyState 1`; `playerStore`'s existing one-shot re-resolve then handles it, and `formatRemotePlayError` maps the copy to `player.err.unreachable`.

Reason:
Measured on an Android 14 emulator with the shipped 3.10.0 APK: tapping a song left the player on "loading" forever, and the 10s start timeout produced "playback timed out" with no sound and no retry. Instrumenting the page showed the element was handed `https://music-dl.sayqz.com/api/?source=netease&id=...`, a host that returns NXDOMAIN — the source race had settled on the first script to answer, and the audio probe let it through because its failure path failed *open* (`null` = "can't tell"). Android's WebView does not fire an `error` event for that host: `play()` stays pending, `readyState` stays 0, and `networkState` drops to `NETWORK_EMPTY`, so the element alone can never distinguish a dead source from a slow one. The old code trusted the element, so the dead source cost the full timeout and the retry that would have picked another source never ran. Failing closed only on a *connection-level* error is what keeps a CDN that merely blocks probes — or answers slowly — working, and preferring known CDNs means the built-in NetEase resolver, which is the app's own last resort, is no longer racing a coin toss against an aggregator.
